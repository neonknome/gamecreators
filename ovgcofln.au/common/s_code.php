...
var s_account="turnerapac-cnau-prd,turnerapacregional-prd,turnerapac-apacinteractive-prd"
...
s.linkInternalFilters="javascript:,cartoonnetwork.com.au"
...
s.siteID="AU"            // leftmost value in pagename
s.defaultPage="index.jsp"       // filename to add when none exists
s.queryVarsList="show,popup_show"     // query parameters to keep
...
function s_doPlugins(s) {
	...
	if(!s.pageType && !s.pageName)
		s.pageName="AU:Game Creator";

	/* Split up page name into hierarchy variables */
	s.prop5=s.siteID;															//Site 
	if(!s.prop6){
		var valueArray = s.pageName.split(":");
  		if(valueArray[1]!=undefined && valueArray[0]!='http' && valueArray[0]!='https') {
			s.prop6=valueArray[0]+':'+valueArray[1];									//Site Section
			if(valueArray[2]!=undefined && valueArray[2]!=s.defaultPage){
				s.prop7=valueArray[0]+':'+valueArray[1]+':'+valueArray[2];					//Sub Section
				if(valueArray[3]!=undefined && valueArray[3]!=s.defaultPage){
					s.prop8=valueArray[0]+':'+valueArray[1]+':'+valueArray[2]+':'+valueArray[3];	//Sub Sub Section
				} else {
					s.prop8=s.prop7;
				}
			} else {
	...
	if(s.prop6&&!s.channel) s.channel=s.prop6;
