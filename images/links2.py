...
tekst_do_usunięcia = 'http://i.cdn.turner.com/v5cache/CARTOON'

# Funkcja do czyszczenia tekstu (usuwanie białych znaków)
def clean_text(text):
    return re.sub(r'\s+', ' ', text.strip()) if text else text

# Funkcja do usuwania linków (usuwamy tylko fragment URL)
def remove_links_from_text(text, link_to_remove):
    # Sprawdzamy, czy tekst zawiera URL, a jeśli tak, to go usuwamy
    if text:
...
for filename in os.listdir(folder_path):
    if filename.endswith(".xml"):
        ...
        for elem in root.iter():
            if elem.text:
                # Czyścimy tekst, usuwamy link
                cleaned_text = clean_text(elem.text)
                new_text = remove_links_from_text(cleaned_text, tekst_do_usunięcia)
            ...
            for attr, value in elem.attrib.items():
                ...
                new_value = remove_links_from_text(cleaned_value, tekst_do_usunięcia)
